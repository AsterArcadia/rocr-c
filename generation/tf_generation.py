...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("tf_generation.log"),
...
PROGRESS_FILE = "tf_generation_progress.json"
...
def process_target(target_dir: str, target_name: str) -> bool:
    ...
    forget_tf_path = os.path.join(target_dir, "forget_tf.json")
    ...
    # 如果已经存在forget_tf.json，则跳过
    if os.path.exists(forget_tf_path):
        logging.info(f"{target_name} 已存在 forget_tf.json 文件，跳过")
        return True
    ...
    # 保存判断题样本
    try:
        with open(forget_tf_path, 'w', encoding='utf-8') as f:
            json.dump(tf_samples, f, ensure_ascii=False, indent=2)
        logging.info(f"成功保存 {target_name} 的 forget_tf.json")
        return True
    except Exception as e:
        logging.error(f"保存 {forget_tf_path} 时出错: {str(e)}")
        return False

def main():
    """主函数"""
    logging.info("开始生成判断题任务")
