...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("neighbor_mcp_generation.log"),
...
PROGRESS_FILE = "neighbor_mcp_generation_progress.json"
...
def process_target(target_dir: str, target_name: str) -> bool:
    ...
    neighbor_mcp_path = os.path.join(target_dir, "neighbor_mcp.json")
    ...
    # 如果已经存在neighbor_mcp.json，则跳过
    if os.path.exists(neighbor_mcp_path):
        logging.info(f"{target_name} 已存在 neighbor_mcp.json 文件，跳过")
        return True
    ...
    # 保存多选题样本
    try:
        with open(neighbor_mcp_path, 'w', encoding='utf-8') as f:
            json.dump(mcp_samples, f, ensure_ascii=False, indent=2)
        logging.info(f"成功保存 {target_name} 的 neighbor_mcp.json")
        return True
    except Exception as e:
        logging.error(f"保存 {neighbor_mcp_path} 时出错: {str(e)}")
        return False

def main():
    """主函数"""
    logging.info("开始生成邻居知识多选题任务")
