...
for name in "${names[@]}"
do
    mkdir -p ./logs/lora/llama3_8b_instruct/original
    id=$name
    output_dir="./saves/lora/llama3_8b_instruct/original"
    echo $id

    PYTHONPATH=./ WANDB_DISABLED=true CUDA_VISIBLE_DEVICES=6,7 python src/train_bash.py --stage ga \
    --model_name_or_path /data/yexiaotian/models/meta-llama/Meta-Llama-3-8B-Instruct \
    --dataset ${id}_Positive --dataset_dir ./data \
    --output_dir ./saves/lora/llama3_8b_instruct/original --overwrite_cache \
    ...
    --target ${id} 2>&1 | tee ./logs/lora/llama3_8b_instruct/original/${id}.log

  #  Delete everything except specified files
