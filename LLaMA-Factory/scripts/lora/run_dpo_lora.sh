...
for name in "${names[@]}"
do
    mkdir -p ./logs/lora/dpo_lora
    id=$name
    echo $id

    PYTHONPATH=./ WANDB_DISABLED=true CUDA_VISIBLE_DEVICES=0,1 python src/train_bash.py --stage dpo \
    ...
    --output_dir ./saves/lora/llama3_8b_instruct/dpo_lora --overwrite_cache \
    ...
    --output_result_dir ./results/lora/llama3_8b_instruct/dpo_lora \
    --fp16 --eval_dataset_dir ./data/unlearn_bench/People/ \
    --target ${id} 2>&1 | tee ./logs/lora/ga_lora/${id}.log
