...
for name in "${names[@]}"; do
  mkdir -p ./logs/lora/llama3_8b_instruct/ga
  id=$name
  output_dir="./saves/lora/llama3_8b_instruct/ga"
  echo $id

  PYTHONPATH=./ WANDB_DISABLED=true CUDA_VISIBLE_DEVICES=0,1 python src/train_bash.py --stage ga \
    --model_name_or_path /data/yexiaotian/models/meta-llama/Meta-Llama-3-8B-Instruct --do_train \
    --dataset ${id}_Positive --dataset_dir ./data --finetuning_type lora --lora_target q_proj,v_proj \
    --output_dir ./saves/lora/llama3_8b_instruct/ga --overwrite_cache \
    ...
    --target ${id} 2>&1 | tee ./logs/lora/llama3_8b_instruct/ga/${id}.log
