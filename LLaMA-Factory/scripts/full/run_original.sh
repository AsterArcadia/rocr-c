...
for name in "${names[@]}"
do
    mkdir -p ./logs/RWKU/Target/original
    id=$name
    echo $id
    ...
    --output_dir ./saves/RWKU/Target/${id}/original/llama3_8b_instruct --overwrite_cache \
    ...
    --output_result_dir ./results/RWKU/Target/${id}/original/llama3_8b_instruct \
    --fp16 --eval_dataset_dir ./data/RWKU/Target/ \
    --target ${id} 2>&1 | tee ./logs/RWKU/Target/original/llama3_8b_instruct_${id}.log
done
