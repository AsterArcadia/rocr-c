...
def eval_forget(model, tokenizer, level1, level2, level3, batch_size=1, output_result_dir=None, use_prompt=False):
    ...
    for answer, question in zip(outputs, questions):
        ...
        if question['level'] == '1':
            ...
            question['prediction'] = answer.strip()
    rouge = Rouge()
    rouge_score_level1 = rouge.get_scores(
        hyps=level1_prediction, refs=level1_answer, avg=True)
    rouge_score_level2 = rouge.get_scores(
        hyps=level2_prediction, refs=level2_answer, avg=True)
    rouge_score_level3 = rouge.get_scores(
        hyps=level3_prediction, refs=level3_answer, avg=True)
