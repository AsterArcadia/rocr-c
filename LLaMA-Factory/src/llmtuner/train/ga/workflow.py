...
def run_ga(
    ...
    with torch.no_grad():
        ...
        eval_forget(model, e_tokenizer, forget_level1, forget_level2, forget_level3, batch_size=16,
                    output_result_dir=os.path.join(output_result_dir, 'forget.json'), use_prompt=data_args.use_prompt)
        print("Evaluate neighbor...")
        eval_neighbor(model, e_tokenizer, neighbor_level1, neighbor_level2, batch_size=16, output_result_dir=os.path.join(
            output_result_dir, 'neighbor.json'), use_prompt=data_args.use_prompt)
        
        print("Evaluate subtoken forget prob...")
        eval_subtoken_forget_prob(model, e_tokenizer, forget_level1, forget_level2, forget_level3, batch_size=1,
                                  output_result_dir=os.path.join(output_result_dir, 'subtoken_forget_prob.json'), use_prompt=data_args.use_prompt)
        print("Evaluate forget prob...")
        eval_forget_prob(model, e_tokenizer, forget_level1, forget_level2, forget_level3, forget_mcp, batch_size=1,
                         output_result_dir=os.path.join(output_result_dir, 'forget_prob.json'), use_prompt=data_args.use_prompt)
        print("Evaluate neighbor prob...")
        eval_neighbor_prob(model, e_tokenizer, neighbor_level1, neighbor_level2, neighbor_mcp, batch_size=1,
                         output_result_dir=os.path.join(output_result_dir, 'neighbor_prob.json'), use_prompt=data_args.use_prompt)
        print("Evaluate subtoken neighbor prob...")
        eval_subtoken_neighbor_prob(model, e_tokenizer, neighbor_level1, neighbor_level2, batch_size=1,
                                  output_result_dir=os.path.join(output_result_dir, 'subtoken_neighbor_prob.json'), use_prompt=data_args.use_prompt)
        print("Evaluate mmlu...")
        eval_mmlu(model, e_tokenizer, retain_mmlu, batch_size=1, output_result_dir=os.path.join(
            output_result_dir, 'mmlu.json'), use_prompt=data_args.use_prompt)
        print("Evaluate truthful...")
        eval_truthfulqa(model, e_tokenizer, truthfulqa, batch_size=4, output_result_dir=os.path.join(
            output_result_dir, 'truthful.json'), use_prompt=data_args.use_prompt)
        print("Evaluate triviaqa...")
        eval_triviaqa(model, e_tokenizer, triviaqa, batch_size=16, output_result_dir=os.path.join(
            output_result_dir, 'triviaqa.json'), use_prompt=data_args.use_prompt)
        print("Evaluate fluency...")
        eval_fluency(model, e_tokenizer, fluency, batch_size=8, output_result_dir=os.path.join(
