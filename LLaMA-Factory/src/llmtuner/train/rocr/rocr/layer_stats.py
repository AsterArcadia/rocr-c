...
def layer_stats(
    ...
    """
    if stat_source not in {"in", "out"}:
        raise ValueError(f"Unsupported stat_source={stat_source}")
    ...
    source_suffix = "" if stat_source == "in" else f"_{stat_source}"
    file_extension = f"{model_name}/{ds_name}_stats/{layer_name}{source_suffix}_{precision}_{'-'.join(sorted(to_collect))}{size_suffix}.npz"
        ...
        for batch_group in progress(loader, total=batch_count):
            for batch in batch_group:
                ...
                with Trace(
                    model,
                    layer_name,
                    retain_input=(stat_source == "in"),
                    retain_output=(stat_source == "out"),
                    stop=True,
                ) as tr:
                    model(**batch)
                traced = tr.input if stat_source == "in" else tr.output
                feats = flatten_masked_batch(traced, batch["attention_mask"])
